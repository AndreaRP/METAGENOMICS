...
bac16SDB="${bacDB}16S/bwt2_offrate/bacteria_offrate"
...
bowtie2 -a -p $threads -fr -x "$bac16SDB" -q -1 $noHostR1Fastq -2 $noHostR2Fastq -S $mappedSam16SFile 2>&1 | tee -a $bowtie2logFile16S
...
bowtie2 -a -p $threads -fr -x "$bacWGDB" -q -1 $noHostR1Fastq -2 $noHostR2Fastq -S $mappedSamWGFile 2>&1 | tee -a $bowtie2logFileWG
