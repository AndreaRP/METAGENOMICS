...
preprocessedFilesDir="${workingDir}/ANALYSIS/02-preprocessing/${sampleName}/" # directory where the preprocessed files are.
...
noHostFilesDir="${workingDir}/ANALYSIS/04-noHost/${sampleName}/" #directory where the host filtering files will we saved (sam for mapping and fastq for host free samples)
