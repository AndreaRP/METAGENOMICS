...
# then
# 	toDo=["copy_utilities" "info_file" "quality_report" "per_sample" "summary"]
# 	#in=$(awk "NR==$SGE_TASK_ID" $toDo)
# 	for i in $toDo
# 	do
# 		$($toDo[i]) &
# 	done
...
function copy_utilities {
	...
	echo -e "The commands are:\ncp -r ${analysisDir}/SRC/html/css* ${resultsDir}\ncp -r ${analysisDir}/SRC/html/img* ${resultsDir}\ncp -r ${analysisDir}/SRC/html/js* ${resultsDir}
	" > $lablog
...
function per_sample {
	...
	echo -e "bash ${analysisDir}/SRC/createSamplesHtml.sh" >> $lablog
	bash ${analysisDir}/SRC/createSamplesHtml.sh 2>&1 | tee -a $lablog
...
function summary {
	...
	echo -e "${analysisDir}/SRC/createSummaryHtml.sh" >> $lablog
